...
class BiLSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # 直接通过Pytorch的LSTM层中的参数，来定义双向LSTM模型
        # 向LSTM层，传入bidirectional = True参数
    ...
    def forward(self, input):
        # 调用一次lstm层，就可以直接计算出正向和逆向的隐藏层结果
        _, (ht, ct) = self.lstm(input)
        # 合并正向和逆向两个方向的隐藏状态
        combined = torch.cat((ht[-2,:,:], ht[-1,:,:]), dim=1)
        output = self.h2o(combined) #计算输出
        return output #将其返回
...
if __name__ == '__main__':
    ...
    model = BiLSTMModel(n_letters, n_hidden, n_classes) # 模型对象
    ...
    torch.save(model.state_dict(), 'names.classify.pytorch.bilstm2')
