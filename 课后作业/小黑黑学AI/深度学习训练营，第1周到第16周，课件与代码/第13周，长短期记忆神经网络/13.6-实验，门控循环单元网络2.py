...
if __name__ == '__main__':
    ...
    # 创建一个GRUModel模型
    model = GRUModel(n_letters, n_hidden, n_classes)
    # 读取已经训练好的模型文件names.classify.gru
    model.load_state_dict(torch.load('names.classify.gru'))
    ...
    for (data, label) in test_loader:
        ...
        letters_num = input.size()[0]
        # 使用模型预测样本的结果
        hidden, output = model(input[0], hidden)
