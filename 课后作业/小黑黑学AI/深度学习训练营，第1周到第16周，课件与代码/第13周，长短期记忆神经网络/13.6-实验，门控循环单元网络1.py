...
# 定义类GRUModel，它继承了torch.nn中的Module模块
class GRUModel(nn.Module):
    # init函数传入输入层、隐藏层和输出层中的神经元数量
    def __init__(self, input_size, hidden_size, output_size):
        super(GRUModel, self).__init__()
        # 隐藏层神经元的个数，会在初始创建隐藏层输出向量时使用
    ...
    def forward(self, input, hidden):
        ...
        # 计算重置门输出
        rt = torch.sigmoid(self.i2h_reset(combined))
        # 计算更新门输出
        zt = torch.sigmoid(self.i2h_update(combined))
        # 计算候选隐藏层状态
        combined = torch.cat((input, rt * hidden), 1)
        candidate = torch.tanh(self.i2h_candidate(combined))
        # 使用zt、hidden和candidate，计算当前时刻的隐藏状态ht
        ht = zt * hidden + (1 - zt) * candidate
        # 使用线性层h2o，计算当前时刻的输出层输出output
        output = self.h2o(ht)
        return ht, output # 返回ht和output
...
if __name__ == '__main__':
    ...
    model = GRUModel(n_letters, n_hidden, n_classes)  # 模型对象
    ...
    for epoch in range(10):  # 外层循环，代表了整个训练数据集的遍历次数
        ...
        for batch_idx, (data, label) in enumerate(train_load):
            ...
            # 前向传播:
            hidden, output = model(tensor[0], hidden)
            # 使用循环计算输入序列tensor的结果
    ...
    torch.save(model.state_dict(), 'names.classify.gru')
