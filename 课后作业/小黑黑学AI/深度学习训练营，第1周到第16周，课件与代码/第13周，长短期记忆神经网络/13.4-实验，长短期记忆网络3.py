...
if __name__ == '__main__':
    ...
    # 创建一个LSTMModel模型
    model = LSTMModel(n_letters, n_hidden, n_classes)
    # 读取已经训练好的模型文件names.classify.lstm
    model.load_state_dict(torch.load('names.classify.lstm'))
    ...
    for (data, label) in test_loader:
        ...
        # 使用模型预测样本的结果
        hidden, cell, output = model(input[0], hidden, cell)
        for i in range(1, letters_num):
            hidden, cell, output = model(input[i], hidden, cell)
