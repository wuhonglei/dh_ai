...
def train(train_df: pd.DataFrame, test_df: pd.DataFrame, title_name: str, label_name: str, feature_method: str):
    ...
    X_train = vectorizer.fit_transform(train_df[title_name])  # 默认按照空格切分
...
for feature_method in methods:
    for title_name in tqdm(title_names, desc=f'训练模型: {feature_method}', total=len(title_names) * len(methods)):
        train(valid_df, test_df, title_name, label_name, feature_method)

df = pd.DataFrame(results)
df.to_csv('./results/base_model/naive_bayes_results.csv', index=False)
