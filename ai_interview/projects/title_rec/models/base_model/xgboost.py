...
def train_xgboost(train_df: pd.DataFrame, test_df: pd.DataFrame, title_name: str, label_name: str, feature_method: str):
...
for feature_method in methods:
    for title_name in tqdm(title_names, desc=f'训练模型: {feature_method}', total=len(title_names) * len(methods)):
        train_xgboost(valid_df, test_df, title_name,
                      label_name, feature_method)

df = pd.DataFrame(results)
df.to_csv('./results/base_model/xgboost_results.csv', index=False)
